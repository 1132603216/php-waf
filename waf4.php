...
define("WAFDIR", "../waf/");   //设置 waf 存储文件上传的目录
define("LOGFILENAME", "req.log");    // 记录请求的文件位置
//时间戳
$timestamp = time();

// 指定filename存储文件
function save_file_upload($filename)
{
    return file_put_contents($filename, file_get_contents($_FILES["file"]["tmp_name"]));
}
...
{
    global $timestamp;
    return WAFDIR . $timestamp . "-" . $filename . "-.txt";
}

// 处理文件上传
//修改文件名并保存上传文件, 并处理文件内容为空
function save_file()
{
    if ($_FILES) {
        global $timestamp;
        $filename = get_filename($_FILES["file"]["name"]);   // 获取新文件名
        save_file_upload($filename);    // 保存文件至waf目录以txt存储
        if (!file_put_contents($_FILES["file"]["tmp_name"], date("Y/m/d H:i:s", $timestamp))) {
//        如果修改内容不成功则修改后缀
    ...
    for ($i = 0; $i < strlen($str); $i++) {
        ...
        if ($ascii > 126 || $ascii < 32) { //有中文这里要修改
            if (!in_array($ascii, array(9, 10, 13))) {
                write_attack_log("interrupt");   // 记录
            } else {
                $str = str_replace($ascii, " ", $str);  // 替换
            }
        }
    }
...
function write_attack_log($alert)
{
    ...
    $ffff = fopen(LOGFILENAME, 'a'); //日志路径
    fwrite($ffff, $data);
    fclose($ffff);
    if ($alert == 'GETFLAG') {
        echo "HCTF{VmpJd2VFNUhSa2RqUkZaWFZrVTFVMWxXV2xabGJGWnlVbGQ0VGsxR1JuTlVWRUpTU2xST1JRJTNEJTNE}"; //如果请求带有flag关键字，显示假的flag。（2333333）
    } else {
        sleep(2); //拦截前延时15秒
    }
    exit(0);
...
filter_attack_keyword(filter_invisible(urldecode(filter_0x25($payload)))); //对POST的内容进行检测，出现问题拦截并记录
